...
2. Copy the ssh URL from the blue Gitlab "Clone" button on the
   ...
   .. code-block:: bash
      ...
      ssh://git@re-git.lanl.gov:10022/aea/stub-repositories/micromorphic_linear_elasticity.git
...
4. Clone the stub repository using the URL copied in step 2.
   ...
   .. code-block:: bash
      ...
      $ git clone ssh://git@re-git.lanl.gov:10022/aea/stub-repositories/micromorphic_linear_elasticity.git
...
5. Rename the local repository directory for your project.
   ...
   .. code-block:: bash
      ...
      $ ls micromorphic_linear_elasticity -d
      micromorphic_linear_elasticity

      # Rename the stub repo directory after your project
      $ mv micromorphic_linear_elasticity my_project

      # Observe that the stub repo directory no longer exists
      $ ls micromorphic_linear_elasticity -d
      ls: cannot access 'micromorphic_linear_elasticity': No such file or directory
...
3. On the "Create new project" page, follow the link for "Create blank project".
   ...
   .. note::
      ...
      guide much simpler in the future. Contact the ``micromorphic_linear_elasticity`` developers and `AEA
...
6. Remove any issue branches from the ``micromorphic_linear_elasticity`` project on the "Repository" >
...
7. If everything looks correct on Gitlab project, you can clean up your local
   ...
   .. warning::
      ...
      micromorphic_linear_elasticity developers for help.

   .. code-block:: bash

      # Remove the micromorphic_linear_elasticity remote
      $ git remote remove old-origin

      # Ensure that you're on the main branch
      $ git checkout main

      # Remove ALL micromorphic_linear_elasticity branches except main and dev
...
The ``micromorphic_linear_elasticity`` project comes pre-configured to perform continuous integration
...
https://aea.re-pages.lanl.gov/stub-repositories/micromorphic_linear_elasticity. This job should be
...
2. Search for all instances of ``micromorphic_linear_elasticity``. The list of occurrences will look
   ...
   .. code-block:: bash
      ...
      $ grep -ri micromorphic_linear_elasticity . --exclude-dir={build,.git} | wc -l
      57

      # Recursive, case-insensitive search and display
      $ grep -ri micromorphic_linear_elasticity . --exclude-dir={build,.git}
      ...
      ...
      ./README.md
      ./set_vars.sh
      ./src/cpp/micromorphic_linear_elasticity.cpp
      ./src/cpp/micromorphic_linear_elasticity.h
      ./src/cpp/tests/test_micromorphic_linear_elasticity.cpp
...
6. Find files containing the project in their file name
   ...
   .. code-block:: bash
      ...
      $ pwd
      /projects/<moniker>/w13repos/my_project
      $ find . -type d \( -name .git -o -name build \) -prune -false -o -name "*micromorphic_linear_elasticity*"
      ./src/cpp/micromorphic_linear_elasticity.cpp
      ./src/cpp/micromorphic_linear_elasticity.h
      ./src/cpp/tests/test_micromorphic_linear_elasticity.cpp
...
7. Rename files after current project
   ...
   .. code-block:: bash
      ...
      $ rename micromorphic_linear_elasticity myproject $(find . -type d \( -name .git -o -name build \) -prune -false -o -name "*micromorphic_linear_elasticity*")
...
8. Commit and push your changes to your "remote" or "fork" repository
   ...
   .. code-block:: bash
      ...
      $ git commit -a -m "FEAT: replace micromorphic_linear_elasticity with my_project throughout repository"
